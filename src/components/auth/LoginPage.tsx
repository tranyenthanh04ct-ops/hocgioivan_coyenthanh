...
import { FIREBASE_AUTH_PROVIDERS_URL } from '../../lib/authUtils';
...
export const LoginPage: React.FC = () => {
  ...
  // Teacher inputs
  const [teacherEmail, setTeacherEmail] = useState('tranyenthanh.04.ct@gmail.com');
  ...
  const [showFirebaseActivationHelp, setShowFirebaseActivationHelp] = useState(false);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setErrorMessage(null);
    setShowFirebaseActivationHelp(false);
    setIsLoading(true);
    ...
    try {
      if (activeTab === 'student') {
        ...
        if (!res.success) {
          if (res.code === 'auth/operation-not-allowed') {
            setShowFirebaseActivationHelp(true);
          }
        ...
        if (!teacherEmail.trim() || !teacherPassword.trim()) {
          setErrorMessage('Vui lòng nhập đầy đủ email quản trị và mật khẩu.');
          setIsLoading(false);
          return;
        }
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md relative z-10">
        ...
        <div className="bg-[#FFFDF9] py-8 px-6 sm:px-10 shadow-xl shadow-rose-950/5 rounded-3xl border border-rose-100">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div>
                  ...
                  <div className="relative rounded-2xl shadow-2xs">
                    ...
                    <input
                      ...
                      placeholder="Ví dụ: haianh hoặc hs01..."
              ...
              <>
                ...
                <div>
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2.5 py-3 px-4 rounded-2xl text-sm font-semibold text-gray-800 bg-white border border-gray-200 hover:bg-gray-50/80 shadow-xs transition-all active:scale-98 disabled:opacity-50"
                  >
                    {isGoogleLoading ? (
                      <div className="flex items-center gap-2">
                      ...
                      <>
                        ...
                        <span>Đăng nhập nhanh bằng Google (1 chạm)</span>
                      </>
                    )}
                  </button>
                  <p className="mt-1 text-center text-[11px] text-gray-400">
                    Khuyên dùng: Đăng nhập tức thì với tranyenthanh.04.ct@gmail.com
                  </p>
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    >
                      Mật khẩu quản trị (1980)
                    </label>
                    <button
                      type="button"
                      onClick={() => setShowFirebaseActivationHelp(!showFirebaseActivationHelp)}
                      className="text-[11px] text-rose-600 hover:text-rose-700 hover:underline"
                    >
                      {showFirebaseActivationHelp ? 'Đóng hướng dẫn' : 'Lỗi đăng nhập 1980?'}
                    </button>
                  </div>
                  <div className="relative rounded-2xl shadow-2xs">
                    <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-gray-400">
                    ...
                    <input
                      ...
                      placeholder="Nhập mật khẩu 1980..."
                      className="block w-full pl-10 pr-10 py-3 bg-white border border-rose-100 rounded-2xl text-sm placeholder-gray-400 focus:outline-hidden focus:ring-2 focus:ring-rose-400 focus:border-transparent transition-all font-mono"
                    />
                    <button
                      type="button"
            ...
            {showFirebaseActivationHelp && (
              <div 
                id="firebase-activation-help-card"
                className="p-4 rounded-2xl bg-amber-50/90 border border-amber-200 text-xs text-amber-900 space-y-2.5 animate-in fade-in duration-200"
              >
                <div className="flex items-start gap-2">
                  <AlertCircle className="w-4 h-4 text-amber-600 shrink-0 mt-0.5" />
                  <div className="space-y-1.5">
                    <p className="font-bold text-amber-900">
                      Cách kích hoạt đăng nhập Mật khẩu 1980 trên Firebase
                    </p>
                    <p className="text-amber-800 leading-relaxed text-[12px]">
                      Dự án Firebase khi mới khởi tạo mặc định chỉ cho phép đăng nhập qua Google. Để đăng nhập bằng Email và mật khẩu <strong>1980</strong>, Cô chỉ cần bật công tắc <strong>Email/Password</strong> một lần duy nhất:
                    </p>
                    <ol className="list-decimal list-inside space-y-1 text-amber-900 text-[11px] font-medium bg-white/70 p-2.5 rounded-xl border border-amber-200/70">
                      <li>Bấm vào nút liên kết bên dưới để mở Firebase Console</li>
                      <li>Chọn <strong>Email/Password</strong> trong danh sách Sign-in providers</li>
                      <li>Bật công tắc <strong>Enable</strong> đầu tiên (không cần bật Email link) rồi bấm <strong>Save</strong></li>
                    </ol>
                    <div className="pt-1 flex items-center gap-2">
                      <a
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-amber-600 text-white font-semibold hover:bg-amber-700 transition-colors shadow-xs text-xs"
                      >
                        <span>Mở Firebase Console cài đặt</span>
